...
export async function SupportSection({ locale }: SupportSectionProps) {
  ...
  // Big tiles (Підписка / Донат / Магазин): 36px title, 18px desc.
  // Small tiles (Діями / Аукціон): 24px title, 16px desc.
  const titleCls = "text-title-mobile font-medium text-text-strong";
  const titleSmallCls = "text-[24px] leading-[120%] font-medium text-text-strong";
  const descCls = "mt-2 text-body text-text-primary lg:text-[18px]";
  const descSmallCls = "mt-2 text-body text-text-primary";
  const iconCls =
  ...
  return (
    <section className="section">
      <div className="container-page">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-12 lg:grid-rows-[400px_400px] lg:gap-6">
          {/* Підписка — col 1 row 1 on every breakpoint */}
          <Link
            href={`/${locale}/contact`}
            className={`${tileBase} bg-accent-1 hover:bg-[#fbdc4f] min-h-45 lg:col-span-5 lg:min-h-0`}
          >
          ...
          <div className="hidden rounded-2xl bg-grey-40 p-8 lg:col-span-7 lg:flex lg:flex-col lg:justify-center">
          ...
          <Link
            ...
            className={`${tileBase} bg-accent-2 hover:bg-[#9AC9FF] min-h-45 lg:col-span-3 lg:min-h-0`}
          >
          ...
          <Link
            href={`/${locale}/shop`}
            className={`${tileBase} bg-accent-3 hover:bg-[#9AF3A2] col-span-2 min-h-45 md:col-span-1 md:row-span-2 lg:col-span-6 lg:row-span-1 lg:min-h-0`}
          >
          ...
          <div className="contents lg:col-span-3 lg:flex lg:flex-col lg:gap-6">
            ...
            >
              <h3 className={titleSmallCls}>{t("volunteer")}</h3>
              <p className={descSmallCls}>{t("volunteer_desc")}</p>
            ...
            >
              <h3 className={titleSmallCls}>{t("auction")}</h3>
              <p className={descSmallCls}>{t("auction_desc")}</p>
