...
const MOCK_RELATED: ArticleDetail[] = [
  {
    ...
    body_en:
      "In dozens of cities around the world, activists spoke out against coercion.",
    cover_image: null,
    category: "organiser",
    tags: [],
    is_featured: false,
  ...
  },
  {
    ...
    title_en: "The world protests against forcing Ukraine to capitulate",
    body_ua:
      "У десятках міст по всьому світу активісти виступили проти примусу.",
    ...
    switch (node.type) {
      case "heading": {
        ...
        if (level === 2)
          return (
            <h2
              key={i}
              className="mt-8 mb-4 text-h2 font-bold text-text-strong"
            >
              {text}
            </h2>
          );
        return (
          <h3
            key={i}
            className="my-4 text-[24px] font-medium leading-[120%] text-text-strong"
          >
            {text}
          </h3>
        );
      ...
      case "paragraph":
        return (
          <p
            key={i}
            className="mb-5 text-[16px] leading-relaxed text-text-primary sm:text-[18px]"
          >
      ...
      case "bulletList":
        return (
          <ul
            key={i}
            className="mb-5 list-disc pl-6 text-[16px] leading-snug text-text-primary sm:text-[18px]"
          >
      ...
      case "orderedList":
        return (
          <ol
            key={i}
            className="mb-5 list-decimal pl-6 text-[16px] leading-snug text-text-primary sm:text-[18px]"
          >
            {(node.content ?? []).map((item, j) => (
              <li key={j}>{renderListItem(item)}</li>
            ))}
      ...
      case "image": {
        ...
        return (
          <div
            key={i}
            className="relative mb-4 aspect-4/3 w-full overflow-hidden rounded-2xl bg-secondary-10 md:float-left md:mr-6 md:mb-4 md:w-[45%] lg:w-[42%]"
          >
            {node.attrs?.src ? (
              <Image
                src={node.attrs.src as string}
...
function renderInline(nodes?: TiptapNode[]): React.ReactNode {
  ...
  return nodes.map((node, i) => {
    ...
    if (node.type === "text") {
      ...
      if (isBold && isItalic)
        return (
          <strong key={i} className="text-text-strong">
            <em>{node.text}</em>
          </strong>
        );
      if (isBold)
...
export default async function NewsArticlePage({
  ...
  const cardPreview = (a: ArticleDetail): string => {
    const lead = (a as unknown as Record<string, string | null | undefined>)[
      leadKey
    ];
  ...
  const publishDate = formatDate(
    article.published_at ??
      (article as unknown as { created_at?: string }).created_at ??
      null,
    locale,
  );
  ...
  const leadText =
    (locale === "ua" ? extra.lead_text_ua : extra.lead_text_en) ?? "";
  const postHeroText =
    (locale === "ua" ? extra.post_hero_text_ua : extra.post_hero_text_en) ?? "";
  const outroText =
    (locale === "ua" ? extra.outro_text_ua : extra.outro_text_en) ?? "";

  return (
    <>
      {/* ── Article header + body ────────────────────────────────── */}
      <article className=" pt-6 pb-10 lg:pt-10 lg:pb-16">
        <div className="container-page">
          ...
          {(bodyImage || body) && (
            <div
              className={
                bodyImage
                  ? "grid grid-cols-1 gap-8 md:grid-cols-2 lg:gap-10"
                  : ""
              }
            >
              {bodyImage && (
            ...
            <div className=" grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((a) => (
                <NewsCard
                  ...
                  coverImage={
                    (a as unknown as { body_image?: string | null })
                      .body_image ?? a.cover_image
                  }
